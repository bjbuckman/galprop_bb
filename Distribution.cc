...
double Distribution::min()   //BJB20161104
{

    double min_= -1.*(numeric_limits<double>::min() - 1.);//-1.e30;    //AWS20050624

    if(n_spatial_dimensions==2)
        for(int ir=0; ir<n_rgrid; ir++)
            for(int iz=0; iz<n_zgrid; iz++)
                for(int ip=0; ip<n_pgrid; ip++)
                    if(d2[ir][iz].s[ip]<min_) min_=d2[ir][iz].s[ip];
    ...
    if(n_spatial_dimensions==3)
        for(int ix=0; ix<n_xgrid; ix++)
            for(int iy=0; iy<n_ygrid; iy++)
                for(int iz=0; iz<n_zgrid; iz++)
                    for(int ip=0; ip<n_pgrid; ip++)
                        if(d3[ix][iy][iz].s[ip]<min_) min_=d3[ix][iy][iz].s[ip];
    return min_;
}
