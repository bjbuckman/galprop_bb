...
int  ProductionMatrices::init(string energy_type_,string energy_units_, int debug_, string model_name_,int energy_dispersion_)
{
    ...
    if (energy_type !="total_energy"&&energy_type !="kinetic_energy") {
        cout<<"invalid energy type !"<<endl;
        exit(1);
    }
    if (energy_units!="GeV"         &&energy_units!="MeV"           ) {
        cout<<"invalid energy units!"<<endl;
        exit(1);
    }
    if (  model_name  !="Huang"
...
// see Schildt c++ Complete Reference page 554: note the +1 needed for getline
            ...
            for(int iE_gamma=0; iE_gamma<nE_gamma; iE_gamma++) {
                ...
                if(debug==1) {
                    cout<<"values=";
                    for(int i=0; i<nE_proton; i++)cout<<values[i]<<" ";
                    cout<<endl;
                }
...
// equation 2 of Huang manual note-1.pdf
            ...
            if(debug==2) {
                ...
                for(int iE_proton=0; iE_proton<nE_proton; iE_proton++)cout<< E_proton[iE_proton]<<" ";
                cout<<endl;

                cout<<"gamma  energies (GeV)="<<endl;
                for(int iE_gamma =0; iE_gamma <nE_gamma ; iE_gamma ++)cout<< E_gamma [iE_gamma ]<<" ";
                cout<<endl;
...
// FLUKA matrices
    ...
    if(model_name=="FLUKA") { //AWS20131104
        ...
        for(process=1; process<=2; process++) {
            ...
            cout<<"E_proton_FLUKA = ";
            for(int iE_proton=0; iE_proton<nE_proton_FLUKA; iE_proton++)  cout<<E_proton_FLUKA[iE_proton]<<" ";
            cout<<endl;
            cout<<"E_gamma_FLUKA = ";
            for(int iE_gamma=0; iE_gamma<nE_gamma_FLUKA ; iE_gamma++ ) cout<<E_gamma_FLUKA [iE_gamma]<<" ";
            cout<<endl;
...
int ProductionMatrices::print()
{
    ...
    for(int iE_proton=0; iE_proton<nE_proton; iE_proton++) cout<< prodxsection_p_matrix  [iE_proton]<<" ";
    cout<<endl;

    cout<<" input prodxsection_he_matrix  = ";
    for(int iE_proton=0; iE_proton<nE_proton; iE_proton++) cout<< prodxsection_he_matrix [iE_proton]<<" ";
    cout<<endl;
...
// if(energy_units_=="MeV")debug=1;
    ...
    if(matrices_read!= 1) {
        cout<<" ProductionMatrices:: rebin: matrices have not been not read !"<<endl;
        exit(1);
    }
    ...
    if(energy_units != "GeV" && energy_units != "MeV") {
        cout<<"ProductionMatrices::rebin: energy_units=" <<energy_units<< " must be GeV or MeV !"<<endl;
        exit(-1);
    }
            ...
            if ( E_proton_total_rebin_GeV > m_p) { // catch case where total energy is less than rest mass
                ...
                for(int iE_gamma_rebin =0; iE_gamma_rebin <nE_gamma_rebin ; iE_gamma_rebin++ ) {
                    ...
                    if(model_name=="Kachelriess2" || model_name=="Kamae_Kachelriess" || model_name=="Kamae") {
                        ...
                        valarray<double> param_a;
                        param_a.resize(9);
                        valarray<double> param_b;
                        param_b.resize(8);
                        valarray<double> param_c;
                        param_c.resize(5);
                        valarray<double> param_d;
                        param_d.resize(5);
...
int ProductionMatrices::emissivity(valarray<double> proton_spectrum_rebin, valarray<double> helium_spectrum_rebin )
{
    ...
    if(matrices_rebinned!= 1) {
        cout<<" ProductionMatrices:: emissivity: matrices have not been not rebinned!"<<endl;
        exit(1);
    }
...
int ProductionMatrices::rebin_with_energy_dispersion(valarray<double> E_proton_rebin_, valarray<double> E_gamma_rebin_, valarray<double> E_gamma_true_)
{
    ...
    cout<<"  ProductionMatrices:::rebin_with_energy_dispersion E_gamma_rebin_=";
    for(int i=0; i<E_gamma_rebin_.size(); i++) cout<<E_gamma_rebin_[i]<<" ";
    cout<<endl;
    cout<<"  ProductionMatrices:::rebin_with_energy_dispersion E_gamma_true_ =";
    for(int i=0; i<E_gamma_true_ .size(); i++) cout<<E_gamma_true_ [i]<<" ";
    cout<<endl;
    ...
    for (int iE_proton_rebin=0; iE_proton_rebin<nE_proton_rebin; iE_proton_rebin++) {
        ...
        if(debug==1) {
            ...
            for(int i=0; i<E_gamma_true_.size(); i++)   cout<<gamma_decay_p_matrix_rebin [iE_proton_rebin][i]<<" ";
            cout<<endl;
...
double ProductionMatrices::ppfrag_v02(double ep,double es,int id, int reac, int kamae_QGS)
{
    ...
    */
    ...
    if(ppfrag_read!=1) {
        ppfrag_spec_ini();    // reads data for both versions
        ppfrag_read=1;
    }
    ...
    if(reac<1 || reac>4) {
        cout<<" ProductionMatrices::ppfrag_v02 invalid reac ! :"<<reac<<endl;
        return result;
    }
...
int ProductionMatrices::test()
{
    ...
    */
    ...
    E_gamma_band_min_[ 0]= 0.050;
    E_gamma_band_max_[ 0]= 0.100;//GeV
    E_gamma_band_min_[ 1]= 0.100;
    E_gamma_band_max_[ 1]= 0.200;

    // 2nd quadrant paper bands
    E_gamma_band_min_[ 2]= 0.200;
    E_gamma_band_max_[ 2]= 0.400;
    E_gamma_band_min_[ 3]= 0.400;
    E_gamma_band_max_[ 3]= 0.600;
    E_gamma_band_min_[ 4]= 0.600;
    E_gamma_band_max_[ 4]= 1.000;
    E_gamma_band_min_[ 5]= 1.000;
    E_gamma_band_max_[ 5]= 2.000;
    E_gamma_band_min_[ 6]= 2.000;
    E_gamma_band_max_[ 6]=10.000;

    // another band
    E_gamma_band_min_[ 7]=10.000;
    E_gamma_band_max_[ 7]=20.000;

    // Abdo etal 2009 HI emissivity paper bands
    E_gamma_band_min_[ 8]= 0.100;
    E_gamma_band_max_[ 8]= 0.140;//GeV
    E_gamma_band_min_[ 9]= 0.140;
    E_gamma_band_max_[ 9]= 0.200;
    E_gamma_band_min_[10]= 0.200;
    E_gamma_band_max_[10]= 0.280;
    E_gamma_band_min_[11]= 0.280;
    E_gamma_band_max_[11]= 0.400;
    E_gamma_band_min_[12]= 0.400;
    E_gamma_band_max_[12]= 0.560;
    E_gamma_band_min_[13]= 0.560;
    E_gamma_band_max_[13]= 0.800;
    E_gamma_band_min_[14]= 0.800;
    E_gamma_band_max_[14]= 1.130;
    E_gamma_band_min_[15]= 1.130;
    E_gamma_band_max_[15]= 1.600;
    E_gamma_band_min_[16]= 1.600;
    E_gamma_band_max_[16]= 2.260;
    E_gamma_band_min_[17]= 2.260;
    E_gamma_band_max_[17]= 3.200;
    E_gamma_band_min_[18]= 3.200;
    E_gamma_band_max_[18]= 4.530;
    E_gamma_band_min_[19]= 4.530;
    E_gamma_band_max_[19]= 6.400;
    E_gamma_band_min_[20]= 6.400;
    E_gamma_band_max_[20]= 9.050;

    // Ackermann etal 2011 3rd quadrant paper
    E_gamma_band_min_[21]= 0.100;
    E_gamma_band_max_[21]= 0.140;//GeV
    E_gamma_band_min_[22]= 0.140;
    E_gamma_band_max_[22]= 0.200;
    E_gamma_band_min_[23]= 0.200;
    E_gamma_band_max_[23]= 0.280;
    E_gamma_band_min_[24]= 0.280;
    E_gamma_band_max_[24]= 0.400;
    E_gamma_band_min_[25]= 0.400;
    E_gamma_band_max_[25]= 0.560;
    E_gamma_band_min_[26]= 0.560;
    E_gamma_band_max_[26]= 0.800;
    E_gamma_band_min_[27]= 0.800;
    E_gamma_band_max_[27]= 1.130;
    E_gamma_band_min_[28]= 1.130;
    E_gamma_band_max_[28]= 1.600;
    E_gamma_band_min_[29]= 1.600;
    E_gamma_band_max_[29]= 2.260;
    E_gamma_band_min_[30]= 2.260;
    E_gamma_band_max_[30]= 3.200;
    E_gamma_band_min_[31]= 3.200;
    E_gamma_band_max_[31]= 4.530;
    E_gamma_band_min_[32]= 4.530;
    E_gamma_band_max_[32]= 6.400;
    E_gamma_band_min_[33]= 6.400;
    E_gamma_band_max_[33]= 9.050;
    E_gamma_band_min_[34]= 9.050;
    E_gamma_band_max_[34]=25.600;
...
/////////////// Kachelriess2013 model
    ...
    if(Kachelriess2013==1) {
        ...
        E_gamma_band_min_MeV[0]=10;
        E_gamma_band_max_MeV[0]=20;
        E_gamma_band_min_MeV[1]=20;
        E_gamma_band_max_MeV[1]=30;
        E_gamma_band_min_MeV[2]=30;
        E_gamma_band_max_MeV[2]=40;
        E_gamma_band_min_MeV[3]=40;
        E_gamma_band_max_MeV[3]=50;
        E_gamma_band_min_MeV[4]=50;
        E_gamma_band_max_MeV[4]=60;
