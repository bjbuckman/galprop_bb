...
int Galprop::store_pi0_decay_HII_skymap()
{
    ...
    if (3 == galdef.skymap_format) {
        ...
        fileprefix += "pi0_decay_HII_ring_";
        for (int i_Ring=0; i_Ring<galaxy.hpHIR.nSpectra(); i_Ring++) {
            ...
            filename += ".gz";
            SkymapToFits(galaxy.pi0_decay_HII_hp_skymap[i_Ring], filename, "Energy", "MeV");
        }
        ...
        naxes[2]=galaxy.pi0_decay_HII_skymap.n_zgrid; // number of Galactocentric rings
        ...
        if (galdef.skymap_format != 1) { //!< Old output
                ...
                for    (int i_Ring   =0;        i_Ring <naxes[2];    i_Ring++)
                    for   (int ib       =0;            ib <naxes[1];        ib++)
                        for (int il       =0;            il <naxes[0];        il++) {
                            array[i]+=galaxy.pi0_decay_HII_skymap .d3[il][ib][i_Ring].s[ip] *pow(galaxy.E_gamma[ip],2);
                            i++;
                        }
            ...
            status = store_skymap(&array[0], naxes, "pion_decay_HII_skymap_", crval, cdelt);
        }
        if(galdef.skymap_format == 1 || galdef.skymap_format == 2) { //!< Mapcube output compatible with Glast science tools
            ...
            for (int i_Ring = 0;        i_Ring <naxes[2];    i_Ring++)
                for (int ip =0; ip <naxes[3]; ip++) // IMOS20080114
                    for   (int ib       =0;            ib <naxes[1];        ib++)
                        for (int il       =0;            il <naxes[0];        il++) {
                            array[i]=galaxy.pi0_decay_HII_skymap .d3[il][ib][i_Ring].s[ip];
                            i++;
                        }

            status = store_mapcube_skymap(&array[0], galaxy.E_gamma, galaxy.pi0_decay_HII_skymap.n_zgrid, galaxy.n_E_gammagrid, "pion_decay_HII_mapcube_", true);
        }
