...
int Galprop::create_galaxy()  //AWS20050816
{
    ...
    int stat=0;
    double dzz=0.0001; // average with this resolution in kpc //orig = 0.01
    ...
    if (2 == galdef.n_spatial_dimensions) { // use average at y=0
        ...
        for (int ir = 0; ir < galaxy.n_rgrid; ++ir) {
            ...
            for (int iz = 0; iz < galaxy.n_zgrid; ++iz) {
                ...
                /* Modified by BJB20161020*/
                const double r = galaxy.r[ir];

                // nHI map
                if (galdef.nHI_model==1) {
                    galaxy.n_HI.d2[ir][iz].s[0]=
                        nH_av(galaxy.r[ir], 0.0, galaxy.z[iz], galaxy.dz, dzz, nHI)*galdef.HI_norm; //IMOS20080114
                } else if (galdef.nHI_model==2) {
                    galaxy.n_HI.d2[ir][iz].s[0]=
                        nH_av_3D(galaxy.r[ir], 0.0, galaxy.z[iz], galaxy.dz, dzz, HICubeData, interp_HI_gas_cube)*galdef.HI_norm;
                } else if (galdef.nHI_model==3) {
                    galaxy.n_HI.d2[ir][iz].s[0]=
                        nH_av_3D(galaxy.r[ir], 0.0, galaxy.z[iz], galaxy.dz, dzz, nHI_3D)*galdef.HI_norm;
                } else if (galdef.nHI_model==10) {
                    galaxy.n_HI.d2[ir][iz].s[0]=
                        nH_av_3D(galaxy.r[ir], 0.0, galaxy.z[iz], galaxy.dz, dzz, galdef.gas_model_name, galdef.gas_model_parameters, nHI_3D)*galdef.HI_norm;
                }

                // nCO map
                if (galdef.nH2_model==1) {
                    galaxy.n_H2.d2[ir][iz].s[0]=
                        nH_av(galaxy.r[ir],0.0, galaxy.z[iz], galaxy.dz, dzz, fX_CO(r), nH2)*galdef.CO_norm; //IMOS20080114
                    //		    nH2_av (galaxy.x[ix],galaxy.y[iy],galaxy.z[iz],galaxy.dz,dzz);
                } else if (galdef.nH2_model==2) {
                    galaxy.n_H2.d2[ir][iz].s[0]=
                        nH_av_3D(galaxy.r[ir],0.0, galaxy.z[iz], galaxy.dz, dzz, fX_CO(r), COCubeData, interp_CO_gas_cube)*galdef.CO_norm;
                } else if (galdef.nH2_model==3) {
                    galaxy.n_H2.d2[ir][iz].s[0]=
                        nH_av_3D(galaxy.r[ir],0.0, galaxy.z[iz], galaxy.dz, dzz, fX_CO(r), nH2_3D)*galdef.CO_norm;
                } else if (galdef.nH2_model==10) {
                    galaxy.n_H2.d2[ir][iz].s[0]=
                        nH_av_3D(galaxy.r[ir], 0.0, galaxy.z[iz], galaxy.dz, dzz, galdef.gas_model_name, galdef.gas_model_parameters, nHI_3D)*galdef.CO_norm;
                }

                // nHII map
                if (galdef.nHII_model==102) {
                    galaxy.n_HII.d2[ir][iz].s[0]=
                        nH_av_3D(galaxy.r[ir], 0.0, galaxy.z[iz], galaxy.dz, dzz, HIICubeData, interp_HI_gas_cube)*galdef.HII_norm;
                } else if (galdef.nHII_model==10) {
                    galaxy.n_HII.d2[ir][iz].s[0]=
                        nH_av_3D(galaxy.r[ir], 0.0, galaxy.z[iz], galaxy.dz, dzz, galdef.gas_model_name, galdef.gas_model_parameters, nHI_3D)*galdef.HII_norm;
                } else {
                    galaxy.n_HII.d2[ir][iz].s[0]=
                        nH_av(galaxy.r[ir], 0, galaxy.z[iz], galaxy.dz, dzz, nHII)*galdef.HII_norm;
                }

                /* ----------------------- END BJB20161020-------------------*/

                /*galaxy.n_HI.d2[ir][iz].s[0]=
                    nH_av(galaxy.r[ir], 0, galaxy.z[iz], galaxy.dz, dzz, nHI)*galdef.HI_norm; //IMOS20080114
        ...
        for (int ix = 0; ix < galaxy.n_xgrid; ++ix) {
            ...
            for (int iy = 0; iy < galaxy.n_ygrid; ++iy) {
                ...
                for (int iz = 0; iz < galaxy.n_zgrid; ++iz) {
                    ...
                    /* Modified by ECC20141024 BJB20170116 */

                    // nHI_map
                    if (galdef.nHI_model==1) {
                        galaxy.n_HI.d3[ix][iy][iz].s[0] =
                            nH_av(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, nHI)*galdef.HI_norm; //IMOS20080114
                    } else if (galdef.nHI_model==2) { //3D gas map input
                        galaxy.n_HI.d3[ix][iy][iz].s[0] =
                            nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, HICubeData, interp_HI_gas_cube)*galdef.HI_norm;
                    } else if (galdef.nHI_model==3) {
                        galaxy.n_HI.d3[ix][iy][iz].s[0] =
                            nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, nHI_3D)*galdef.HI_norm;
                    }

                    // nCO_map
                    if (galdef.nH2_model==1) {
                        galaxy.n_H2.d3[ix][iy][iz].s[0] =
                            nH_av(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, fX_CO(r), nH2)*galdef.CO_norm; //IMOS20080114
                        //		    nH2_av (galaxy.x[ix],galaxy.y[iy],galaxy.z[iz],galaxy.dz,dzz);
                    } else if (galdef.nH2_model==2) { //3D gas map input
                        galaxy.n_H2.d3[ix][iy][iz].s[0] =
                            nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, fX_CO(r), COCubeData, interp_CO_gas_cube) *galdef.CO_norm;
                    } else if (galdef.nH2_model==3) {
                        galaxy.n_H2.d3[ix][iy][iz].s[0] =
                            nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, fX_CO(r), nH2_3D)*galdef.CO_norm;
                    }

                    // nHII_map
                    if (galdef.nHII_model==102) { //3D gas map input
                        galaxy.n_HII.d3[ix][iy][iz].s[0] =
                            nH_av_3D(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, HIICubeData, interp_HI_gas_cube)*galdef.HII_norm;
                    } else {
                        galaxy.n_HII.d3[ix][iy][iz].s[0] =
                            nH_av(galaxy.x[ix], galaxy.y[iy], galaxy.z[iz], galaxy.dz, dzz, nHII)*galdef.HII_norm;  //IMOS20080114
                        //		    nHII_av(galaxy.x[ix],galaxy.y[iy],galaxy.z[iz],galaxy.dz,dzz);
                    }
                    /* ----------------------- END ECC20141024 BJB20170116 -------------------*/
