...
double cs_ppd_fusion(double Ekin_d, double Ekin_p, int cs_init, int debug)
{
    ...
    if(debug==1) {
        ...
        if(Wt_prime<M3+M4)
            cout<< " reaction energetically excluded!";
        cout<<endl;
...
double cs_ppd_total(double Ekin_p, int cs_init, int debug)
{
    ...
    */
    ...
    if (cs_init==1) {
        ...
        if(i!=n-1) {
            cout<<"error in cs_ppd_fusion_total data"<<endl;
            exit(1);
        }
    ...
    if(Ekin_p>E[0] && Ekin_p<E[n-2]) {
        ...
        if (debug==1) {
            cout<<"cs_ppd_fusion_total: "<<" Ekin_p="<<Ekin_p<<" i="<<i<< " E[i]="<<E[i]<<" E[i+1]="<<E[i+1]
