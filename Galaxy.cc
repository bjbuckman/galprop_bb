...
int Galaxy::count_grid_points(double min, double max, double d, double min_inner, double max_inner, double d_inner)
{
    int count = 0;
    double position = min;
    while (position<=max) {
        if ((position >=min) && (position <= max)) {
            // Check if we are in the high resolution box
            if ((position >= min_inner) && (position < max_inner))
                position += d_inner;
            else
                position += d;
...
void Galaxy::init( double x_min_, double x_max_, double dx_,
                   ...
                   double x_min_inner_, double x_max_inner_, double dx_inner_,
                   double y_min_inner_, double y_max_inner_, double dy_inner_,
                   double z_min_inner_, double z_max_inner_, double dz_inner_)
{
    ...
    x_min_inner=x_min_inner_;
    x_max_inner=x_max_inner_;
    dx_inner   =   dx_inner_;
    y_min_inner=y_min_inner_;
    y_max_inner=y_max_inner_;
    dy_inner   =   dy_inner_;
    z_min_inner=z_min_inner_;
    z_max_inner=z_max_inner_;
    dz_inner   =   dz_inner_;

    n_xgrid = (int) count_grid_points(x_min, x_max, dx, x_min_inner, x_max_inner, dx_inner);
    n_ygrid = (int) count_grid_points(y_min, y_max, dy, y_min_inner, y_max_inner, dy_inner);
    n_zgrid = (int) count_grid_points(z_min, z_max, dz, z_min_inner, z_max_inner, dz_inner);

    cout << "n_x grid points " << n_xgrid << endl;
    cout << "n_y grid points " << n_ygrid << endl;
    cout << "n_z grid points " << n_zgrid << endl;
    ...
    int ix,iy,iz;
    double position = x_min;

    for(ix=0; ix<n_xgrid; ix++) {
        if (abs(position)<1e-10)
            position=0;
        x[ix] = position;
        if ((position >= x_min_inner) && (position < x_max_inner)) position += dx_inner;
        else position += dx;
        cout << "X-GRID " << ix << " : " << x[ix] << endl;
    }

    position = y_min;
    for(iy=0; iy<n_ygrid; iy++) {
        y[iy] = position;
        if (abs(position)<1e-10)
            position=0;
        if ((position >= y_min_inner) && (position < y_max_inner)) position += dy_inner;
        else position += dy;
        cout << "Y-GRID " << iy << ":" << y[iy] << endl;
    }

    position = z_min;
    for(iz=0; iz<n_zgrid; iz++) {
        z[iz] = position;
        if (abs(position)<1e-10)
            position=0;
        if ((position >= z_min_inner) && (position < z_max_inner)) position += dz_inner;
        else position += dz;
        cout << "Z-GRID " << iz << ":" << z[iz] << endl;
    }



    n_HI.init(n_xgrid, n_ygrid, n_zgrid, 1);
    INFO("FINISHED n_HI init");
    n_H2.init(n_xgrid, n_ygrid, n_zgrid, 1);
    n_HII.init(n_xgrid, n_ygrid, n_zgrid, 1);
    B_field.init(n_xgrid, n_ygrid, n_zgrid, 1);
...
void Galaxy::print()
{
    ...
    if(n_spatial_dimensions==2) {
        ...
        cout<<"r grid:"<<endl;
        for(int ir=0; ir<n_rgrid; cout<<r[ir++]<<" ");
        cout<<endl;
        cout<<"z grid:"<<endl;
        for(int iz=0; iz<n_zgrid; cout<<z[iz++]<<" ");
        cout<<endl;
    ...
    if(n_spatial_dimensions==3) {
        ...
        cout<<"x grid:"<<endl;
        for(int ix=0; ix<n_xgrid; cout<<x[ix++]<<" ");
        cout<<endl;
        cout<<"y grid:"<<endl;
        for(int iy=0; iy<n_ygrid; cout<<y[iy++]<<" ");
        cout<<endl;
        cout<<"z grid:"<<endl;
        for(int iz=0; iz<n_zgrid; cout<<z[iz++]<<" ");
        cout<<endl;
