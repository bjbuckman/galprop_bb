...
static void ReadISRFFormatV3(Galprop& g)
{
    ...
    if (galdef.verbose==-1000) {
        ofstream myfile;
        myfile.open ("isrf_plane.dat");

        //myfile << "r z E opt NIR CMB"<< endl;

        myfile << "nr=" << galaxy.n_rgrid << " nz=" << galaxy.n_zgrid << " nE=" << targetBins << " r z e opt NIR CMB"<< endl;

        for (int i_r = 0; i_r < galaxy.n_rgrid; ++i_r) {
            for (int i_z = 0; i_z < galaxy.n_zgrid; ++i_z) {
                for (int l = 0; l < targetBins; ++l) {

                    double r = galaxy.r[i_r];
                    double z = galaxy.z[i_z];
                    double e = targetE[l];

                    double opt = galaxy.ISRF[0].d2[i_r][i_z].s[l]; // OPT
                    double NIR = galaxy.ISRF[1].d2[i_r][i_z].s[l]; // NIR
                    double CMB = galaxy.ISRF[2].d2[i_r][i_z].s[l]; // CMB
