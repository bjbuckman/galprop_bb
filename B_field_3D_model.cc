...
// notation here as for other models
    ...
    if(name == "m82_01") {
        ...
        double b_field;
        double bz_power;
        double Bo_largescale,r_largescale,z_largescale,ro;
        ...
        Bo_largescale=parameters[4];
        r_largescale=parameters[5];
        z_largescale=parameters[6];
        ro=parameters[7];
        ...
        b_field+=Bo_largescale *exp(-(r-ro)/r_largescale) * exp(-fabs(z)/z_largescale);
    ...
    if(name == "m82_02") {
        ...
        double Bo,rscale,rscale1,zscale,zscale1;
        double b_field;
        double bz_power;
        double Bo_largescale,r_largescale,z_largescale,ro;
        ...
        // now take parameters as specified for 3D model in galdef file           AWS20080314
        Bo   = parameters[0];   // Gauss
        rscale=parameters[1];   // kpc Scale of starburst
        rscale1=parameters[2];
        zscale=parameters[3];   // kpc Scale of starburst
        zscale1=parameters[4];  // kpc Scale of field drop-off outside of starburst
        bz_power=parameters[5]; // power law drop-off outside of starburst

        Bo_largescale=parameters[6]; // Gauss LARGE SCALE
        r_largescale=parameters[7];	 // Large r scale
        z_largescale=parameters[8];  // Large z scale
        ro=parameters[9];            // r large scale normalization point

        if (r <= rscale && fabs(z) <= zscale) {
            b_field = Bo;
        } else if (r <= rscale && fabs(z) > zscale) {
            b_field = Bo*pow( (fabs(z)-zscale+zscale1)/zscale1, bz_power);
        } else if (fabs(z) <= zscale && r > rscale) {
            b_field = Bo*exp(-(r-rscale)/rscale1);
        } else {
            b_field = Bo*exp(-(r-rscale)/rscale1)*pow( (fabs(z)-zscale+zscale1)/zscale1, bz_power);
        }
