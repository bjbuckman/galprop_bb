...
public:
    ...
    {
        return sqrt(3/utl::kPi)*60/nside_;
    }
    /**\brief Solid angle of each pixel in radians */
    double solidAngle() const
    {
        return utl::kPi/(3*nside_*nside_);
    }
    //! Coordinate for a given pixel
    SM::Coordinate pix2coord(int pix) const
    {
        return SM::Coordinate(pix2ang(pix));
    }
    //! Pixel for a given coordinate
    int coord2pix(const SM::Coordinate & coord) const
    {
        return ang2pix(coord.healpixAng());
    }
