        ...
        */
        ...
        for(int ir=0; ir<particle.n_rgrid; ir++) {
            for(int iz=0; iz<particle.n_zgrid; iz++) {
                ...
                double rrrr=sqrt(pow(particle.r[ir],2)+pow(particle.z[iz],2));
                total_sources += sourceval*particle.r[ir];
                if (rrrr <= .1)
                    src_100 += sourceval*particle.r[ir];
                if (rrrr <= .2)
                    src_200 += sourceval*particle.r[ir];
                if (rrrr <= .3)
        ...
        cout << "============================================================" <<  endl;
        cout << "============================================================" <<  endl;
        cout << "Fraction of total sources within 100,200,300,500 pc of GC: "
             << src_100/total_sources << ", "
             << src_200/total_sources << ", "
        ...
        for(int ir=0; ir<particle.n_rgrid; ++ir)
            for(int iz=0; iz<particle.n_zgrid; ++iz)
                for(int ip=particle.n_pgrid-1; ip>=0; --ip) {
                    ...
                    if(galdef.convection==5) { //M82 cylinder BJB161202
                        //cylinder of radius galdef.r0_conv
                        //exponential drop off in r~galdef.r1_conv outside galdef.r0_conv
                        if (particle.r[ir] <= galdef.r0_conv) {

                            //r convection
                            particle.v_conv_x.d2[ir][iz].s[ip] = galdef.v0_conv_x*0.5*(1.0+tanh( 4.0*abs(particle.r[ir]/galdef.r0_conv)-4.0));

                            //z convection
                            particle.v_conv_z.d2[ir][iz].s[ip] = galdef.v0_conv_z*0.5*(1.0+tanh( 4.0*abs(particle.z[iz]/galdef.z0_conv)-4.0));
                            // velocity negative below plane
                            if(particle.z[iz] < 0.)   particle.v_conv.d2[ir][iz].s[ip]*= -1.;

                            //"dvdz_conv" This should work it we find magnitude of derivative. Since reacceration should care about direction.
                            double dvdz_temp = 0;
                            dvdz_temp = pow(galdef.v0_conv_z*0.5*4.0/galdef.z0_conv*pow(cosh(4.0*abs(particle.z[iz]/galdef.z0_conv)-4.0),-2), 2);
                            dvdz_temp+= pow(galdef.v0_conv_x*0.5*4.0/galdef.r0_conv*pow(cosh(4.0*abs(particle.r[ir]/galdef.r0_conv)-4.0),-2), 2);
                            particle.dvdz_conv.d2[ir][iz].s[ip] = sqrt(dvdz_temp);
                        ...
                        if (particle.r[ir] >  galdef.r0_conv) {
                            ...
                            //r convection
                            particle.v_conv_x.d2[ir][iz].s[ip] = galdef.v0_conv_x*0.5*(1.0+tanh( 4.0*abs(particle.r[ir]/galdef.r0_conv)-4.0));
                            particle.v_conv_x.d2[ir][iz].s[ip]*= exp(-1*(particle.r[ir]-galdef.r0_conv)/galdef.r1_conv);

                            //z convection
                            particle.v_conv_z.d2[ir][iz].s[ip] = galdef.v0_conv_z*0.5*(1.0+tanh( 4.0*abs(particle.z[iz]/galdef.z0_conv)-4.0));
                            particle.v_conv_z.d2[ir][iz].s[ip]*= exp(-1*(particle.r[ir]-galdef.r0_conv)/galdef.r1_conv);
                            // velocity negative below plane
                            if(particle.z[iz] < 0.)   particle.v_conv.d2[ir][iz].s[ip]*= -1.;

                            //"dvdz_conv" This should work it we find magnitude of derivative. Since reacceration should care about direction.
                            double dvdz_temp  = 0;
                            double dvdz_tempx1 = 0;
                            double dvdz_tempz1 = 0;
                            double dvdz_tempx2 = 0;
                            double dvdz_tempz2 = 0;
                            dvdz_tempx1 = galdef.v0_conv_x *0.5*4.0/ galdef.r0_conv*pow(cosh(4.0*abs(particle.r[ir]/galdef.r0_conv)-4.0),-2);
                            dvdz_tempx1*= exp(-1*(particle.r[ir]-galdef.r0_conv)/galdef.r1_conv);
                            dvdz_tempx2 = galdef.v0_conv_x *0.5*(1.0+tanh( 4.0*abs(particle.r[ir]/galdef.r0_conv)-4.0));
                            dvdz_tempx2*= -1/galdef.r1_conv*exp(-1*(particle.r[ir]-galdef.r0_conv)/galdef.r1_conv);
                            dvdz_tempz1 = galdef.v0_conv_z *0.5*4.0/ galdef.z0_conv*pow(cosh(4.0*abs(particle.z[iz]/galdef.z0_conv)-4.0),-2);
                            dvdz_tempz1*= exp(-1*(particle.r[ir]-galdef.r0_conv)/galdef.r1_conv);
                            dvdz_tempz2 = galdef.v0_conv_z *0.5*(1.0+tanh( 4.0*abs(particle.z[iz]/galdef.z0_conv)-4.0));
                            dvdz_tempz2*= -1/galdef.r1_conv*exp(-1*(particle.r[ir]-galdef.r0_conv)/galdef.r1_conv);
                            dvdz_temp  = sqrt( pow(dvdz_tempx1+dvdz_tempx2, 2) + pow(dvdz_tempz1+dvdz_tempz2, 2) );
                            particle.dvdz_conv.d2[ir][iz].s[ip] = sqrt(dvdz_temp);

                        }
                    } //BJB161202

                    if(galdef.convection==6) { //M82 radial BJB161202
                        //spherically symmetric wind of scale length galdef.r0_conv
                        //Parameters: v0_conv, r0_conv

                        double rrrr = sqrt(pow(particle.r[ir],2)+pow(particle.z[iz],2));
                        double theta = atan2(particle.z[iz],particle.r[ir]);
                        double v_total = galdef.v0_conv*0.5*(1.0+tanh( 4.0*abs(rrrr/galdef.r0_conv)-4.0));
                        particle.v_conv_x.d2[ir][iz].s[ip] = v_total*cos(theta);
                        particle.v_conv_z.d2[ir][iz].s[ip] = v_total*sin(theta);
                        particle.dvdz_conv.d2[ir][iz].s[ip] = galdef.v0_conv*0.5*4.0/galdef.r0_conv*pow((cosh(4.0*rrrr/galdef.r0_conv)-4.0),-2);
            ...
            for(int iy=0; iy<particle.n_ygrid; ++iy)
                for(int iz=0; iz<particle.n_zgrid; ++iz)
                    for(int ip=particle.n_pgrid-1; ip>=0; --ip) {
                        ...
                        // BJB161202 3D CONVECTION=5 NOT WRITTEN YET!
            ...
            for(int iz=0; iz<particle.n_zgrid; iz++) {
                for(int ip=0; ip<particle.n_pgrid; ip++) {
                    ...
                    if(particle.A!=0) {
                        ...
                        particle.dpdt.d2[ir] [iz].s[ip] = dpdt_ion_coul;

                        // losses due to pion production, not included in nucleon_loss AWS20150412
                        int hadronic_energy_loss_debug=0; //AWS20150412
                        if(galdef.verbose==-6000)  hadronic_energy_loss_debug=1; // selectable debug //AWS20150412

                        // Hadronic losses
                        double dpdt_hadronic =
                            hadronic_energy_loss(particle.A, particle.Etot[ip],
                                                 galaxy.n_HI .d2[ir] [iz].s[0] +2*galaxy.n_H2.d2[ir] [iz].s[0]+ galaxy.n_HII.d2[ir] [iz].s[0], galdef.He_H_ratio,galdef.hadronic_loss_model, hadronic_energy_loss_debug)
                            / particle.beta[ip]*1.0e-6;  // energy eV s-1 -> momentum MeV s-1 //AWS20150412
                        if(galdef.hadronic_losses==1) particle.dpdt.d2[ir] [iz].s[ip]+= dpdt_hadronic; //AWS20150412

                        if(galdef.verbose==-6000) {
                            cout<<"create_tranport_arrays: A="<< particle.A
                                << " Etot="<< particle.Etot[ip]<< " Ekin="<< particle.Ekin[ip]
                                << " r="<<galaxy.r[ir]<< " z="<<galaxy.z[iz]
                                << " dpdt_ion_coul="<<dpdt_ion_coul<<" dpdt_hadronic="<<dpdt_hadronic<<" MeV s-1";
                            if(dpdt_ion_coul> dpdt_hadronic)cout<<" ionization+coulomb loss dominates";
                            if(dpdt_ion_coul< dpdt_hadronic)cout<<"        hadronic    loss dominates";
                            cout<<endl;
                        }
                    }
                    ...
                    // ELECTRONS
                    if(particle.A==0) {
                        double uevcm3=0., bevcm3, brem1,brem2,sync,cmptn;
                        // IMOS200008016
            ...
            for(int iy=0; iy<particle.n_ygrid; iy++) {
                for(int iz=0; iz<particle.n_zgrid; iz++) {
                    for(int ip=0; ip<particle.n_pgrid; ip++) {
                        ...
                        if(particle.A!=0) {
                            ...
                            double dpdt_ion_coul =
                                nucleon_loss(particle.Z,particle.A,particle.Etot[ip],
                                             ...
                                             &aion,  &coul) / particle.beta[ip]*1.0e-6; // energy eV s-1 -> momentum MeV s-1
                            particle.dpdt.d3[ix][iy][iz].s[ip]+= dpdt_ion_coul;
                            ...
                            double dpdt_hadronic =
                                hadronic_energy_loss(particle.A, particle.Etot[ip],
                                                     galaxy.n_HI .d3[ix][iy][iz].s[0] +2*galaxy.n_H2.d3[ix][iy][iz].s[0]+ galaxy.n_HII.d3[ix][iy][iz].s[0], galdef.He_H_ratio,galdef.hadronic_loss_model, hadronic_energy_loss_debug)
                                / particle.beta[ip]*1.0e-6;  // energy eV s-1 -> momentum MeV s-1 //AWS20150412
                            if(galdef.hadronic_losses==1) particle.dpdt.d3[ix][iy][iz].s[ip]+= dpdt_hadronic; //AWS20150412

                            if(galdef.verbose==-6000) {
                                cout<<"create_tranport_arrays: A="<< particle.A
                                    << " Etot="<< particle.Etot[ip]<< " Ekin="<< particle.Ekin[ip]
                                    << " x="<<galaxy.x[ix]<< " y="<<galaxy.y[iy]<< " z="<<galaxy.z[iz]
                                    << " dpdt_ion_coul="<<dpdt_ion_coul<<" dpdt_hadronic="<<dpdt_hadronic<<" MeV s-1";
                                if(dpdt_ion_coul> dpdt_hadronic)cout<<" ionization+coulomb loss dominates";
                                if(dpdt_ion_coul< dpdt_hadronic)cout<<"        hadronic    loss dominates";
                                cout<<endl;
                        ...
                        // ELECTRONS
                        if(particle.A==0) {
                            double uevcm3=0., bevcm3=0.,brem1,brem2,sync,cmptn;
                            // IMOS200008016
