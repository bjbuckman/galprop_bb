...
int Galprop::propel(Particle& particle)
{
    ...
    if (2 == particle.n_spatial_dimensions) { // ==== 2D ====
        ...
        */
        ...
        if (galdef.convection==1 || galdef.convection==2 || galdef.convection==3) {
        ...
        if (galdef.convection==4 || galdef.convection==5 || galdef.convection==6) {
                ...
                for (iz = 0; iz < particle.n_zgrid; ++iz) { // numerator = abs convection velocity in cm s^-1
                    ...
                    for (ip = 0; ip < particle.n_pgrid; ++ip) {
                        ...
                        if (ip == particle.n_pgrid-1) {
                            ap2 = particle.p[ip]  /3.*particle.dvdz_conv.d2[ir][iz].s[ip] / (particle.p[ip]-particle.p[ip-1])*1.e5/kpc2cm;
                            ap3 = 0;
                        } else {
                            ap2 = particle.p[ip]  /3.*particle.dvdz_conv.d2[ir][iz].s[ip] / (particle.p[ip+1]-particle.p[ip])*1.e5/kpc2cm;
                            ap3 = particle.p[ip+1]/3.*particle.dvdz_conv.d2[ir][iz].s[ip] / (particle.p[ip+1]-particle.p[ip])*1.e5/kpc2cm;
                        }
