...
int Galprop::store_gcr_source_functions(Particle &particle)
{
    ...
    if(particle_name=="Hydrogen_1"       )  {
        filename="primary_protons_source_function";
        origin="primary";
    }
    if(particle_name=="Helium_4"         )  {
        filename="primary_Helium_source_function";
        origin="primary";
    }
    if(particle_name=="primary_electrons")  {
        filename="primary_electrons_source_function";
        origin="primary";
    }
    if(particle_name=="primary_positrons")  {
        filename="primary_positrons_source_function";
        origin="primary";
    }

	if(particle_name=="secondary_protons") {
        filename="secondary_protons_source_function";
        origin="secondary";
    }
    if(particle_name=="secondary_electrons") {
        filename="secondary_electrons_source_function";
        origin="secondary";
    }
    if(particle_name=="secondary_positrons") {
        filename="secondary_positrons_source_function";
        origin="secondary";
    }

    buf<<" "<<origin<<" "<<filename<<endl;

    if(origin=="origin_not_assigned"||filename=="filename_not_assigned") {
        INFO(buf.str());
        INFO("Exit");
        status=1;
        return status;
