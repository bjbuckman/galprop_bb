...
double kappa_free_free(double nu,double Ne, double Te, double &emiss_free_free, int options, int debug)
{
    ...
    double kappa_free_free_= kff(nu,Ne,Te);//   0.0178*gff*Ne*Ne/(nu*nu*Te*sqrt(Te));// Allen Astrophysical Quantities 4th Edition Section 5.9
    ...
    if(kappa_free_free_ <= 0) {
        kappa_free_free_ = 1e-50;
    }
    if(emiss_free_free <= 0) {
        emiss_free_free = 1e-50;
    }
...
// double kappa_free_free_2D(double nu, int ir, int iz,  double Te, double clumping_factor, double &emiss_free_free, int options, int debug)
// {
// // AWS20110630
// // free-free absorption coefficient in cm^-1 at position in Galaxy
// // based on model for free-electron distribution
...
// double Ne = galaxy.n_HII.d2[ir][iz].s[0];
...
// double kappa_free_free_3D(double nu, int ix, int iy, int iz,  double Te,  double clumping_factor, double &emiss_free_free, int options, int debug)
// {
// // AWS20110630
// // free-free absorption coefficient in cm^-1 at position in Galaxy
// // based on model for free-electron distribution
...
// double Ne = galaxy.n_HII.d3[ix][iy][iz].s[0];
...
// {
// cout<<" kappa_free_free_3D for free electron distribution at:"<<endl;
// cout<<" x="<<galaxy.x[ix]<<endl;
// cout<<" y="<<galaxy.y[iy]<<endl;
// cout<<" z="<<galaxy.z[iz]<<endl;
// cout<<" nu="<<nu<<endl;
// cout<<" Ne="<<Ne<<endl;
// cout<<" Te="<<Te<<endl;
