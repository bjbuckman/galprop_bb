...
// //cout<<"cr_luminosity:  ip= "<<ip<<" CR_luminosity="<<  CR_luminosity<<endl;
    ...
    // *log10(gcr[iprotons].Ekin[1]/ gcr[iprotons].Ekin[0]) ;
...
// identify secondary CR protons
    int isprotons=-1;
	int tixx = 0;
    for(int i=n_species-1; i>=0; i--) {
		if(gcr[i].A==1 && gcr[i].Z==1) {
			tixx=tixx+1;
			isprotons=i;
		}
	}
    if(isprotons==-1||tixx==1) {
        WARNING("CR secondary protons not found!");
        return 1;
    } else {
        buf.str("");
        buf<<"  CR secondary protons found as species #"<<isprotons;
        INFO(buf.str());
    }

    if (galdef.output_gcr_full) {
        //AWS20100311
        particle=gcr[isprotons];
        create_transport_arrays(particle);
...
//cout<<"cr_luminosity:  ip= "<<ip<<" CR_luminosity="<<  CR_luminosity<<endl;
        ...
        CR_luminosity *= 4.0*Pi/c *1.0e6 *eV_to_erg *pow(kpc2cm,3)
                         *log10(gcr[ielectrons].Ekin[1]/ gcr[ielectrons].Ekin[0]) ;
