//**.****|****.****|****.****|****.****|****.****|****.****|****.****|****.****|
// * store_bremss_H2R_skymap.cc *                     galprop package * 4/14/2000
//**"****!****"****!****"****!****"****!****"****!****"****!****"****!****"****|
...
int Galprop::store_bremss_H2R_skymap()
{
    ...
    if (3 == galdef.skymap_format) {
        ...
        for (int i_Ring=0; i_Ring<galaxy.hpHIR.nSpectra(); i_Ring++) {
            ...
            filename += ".gz";
            SkymapToFits(galaxy.bremss_H2R_hp_skymap[i_Ring], filename, "Energy", "MeV");
        }
        ...
        naxes[2]=galaxy.bremss_H2R_skymap.n_zgrid; // number of Galactocentric rings
        ...
        if (galdef.skymap_format != 1) { //!< Old output
                ...
                for    (int i_Ring   =0;        i_Ring <naxes[2];    i_Ring++)
                    for   (int ib       =0;            ib <naxes[1];        ib++)
                        for (int il       =0;            il <naxes[0];        il++) {
                            array[i]=0.0;
                            array[i]+=galaxy.bremss_H2R_skymap .d3[il][ib][i_Ring].s[ip] *pow(galaxy.E_gamma[ip],2);
                            i++;
                        }
            ...
            status = store_skymap(&array[0], naxes, "bremss_H2R_skymap_", crval, cdelt);
        ...
        if(galdef.skymap_format == 1 || galdef.skymap_format == 2) { //!< Mapcube output compatable with Glast science tools
            ...
            for       (int i_Ring   =0;        i_Ring <naxes[2];    i_Ring++)
                for     (int ip       =0;            ip <naxes[3];        ip++) // IMOS20080114
                    for   (int ib       =0;            ib <naxes[1];        ib++)
                        for (int il       =0;            il <naxes[0];        il++) {
                            array[i]=galaxy.bremss_H2R_skymap .d3[il][ib][i_Ring].s[ip];
                            i++;
                        }

            status = store_mapcube_skymap(&array[0], galaxy.E_gamma, galaxy.bremss_H2R_skymap.n_zgrid, galaxy.n_E_gammagrid, "bremss_H2R_mapcube_", true);
        }
