...
int Galprop::store_freefree()                  //BJB20170308
{
    ...
    for(int ff=0; ff<=1; ff++) { //AWS20100708
        int i = 0;
        if (2 == galaxy.n_spatial_dimensions) {
            for (int ip = 0; ip < naxes[2]; ++ip)
                for (int iz = 0; iz < naxes[1]; ++iz)
                    for (int ir = 0; ir < naxes[0]; ++ir) {
                        if(ff==0)array[i] = galaxy.free_free_emiss .d2[ir][iz].s[ip];
                        if(ff==1)array[i] = galaxy.free_free_absorb.d2[ir][iz].s[ip];
                        ++i;
                    } //ir,iz,ip
        } //2D
                ...
                for (int iz = 0; iz < naxes[2]; ++iz)
                    for (int iy = 0; iy < naxes[1]; ++iy)
                        for (int ix = 0; ix < naxes[0]; ++ix) {
                            if(ff==0)array[i] = galaxy.free_free_emiss .d3[ix][iy][iz].s[ip];
                            if(ff==1)array[i] = galaxy.free_free_absorb.d3[ix][iy][iz].s[ip];
                            ++i;
                        } //ix,iy,iz,ip
        } //3D

        string outfile;
        if(ff==0)	outfile = "!" + configure.fOutputDirectory + configure.fOutputPrefix + "free_free_emiss_"   + galdef.galdef_ID + ".gz";
        if(ff==1)	outfile = "!" + configure.fOutputDirectory + configure.fOutputPrefix + "free_free_absorb_" + galdef.galdef_ID + ".gz";
