                              ...
                              int debug=0 )
{
    ...
    */
    ...
    if(Bperp>0.0) {
        ...
        double F=0.0;
        status= gsl_sf_synchrotron_1_e(x,&result) ;
        if(status==0) F=result.val;
        double G=0.0;
        status= gsl_sf_synchrotron_2_e(x,&result) ;
        if(status==0) G=result.val;
    ...
    if(Brand>0.) {
        ...
        double K13=0.0;
        status = gsl_sf_bessel_Knu_e (1./3.,  x, &result);
        if(status==0) K13=result.val; // modified Bessel function of order 4/3
        double K43=0.0;
        status = gsl_sf_bessel_Knu_e (4./3.,  x, &result);
        if(status==0) K43=result.val; // modified Bessel function of order 4/3
