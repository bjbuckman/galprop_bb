     ...
     char *energy_units_)
{
    ...
    strcpy(area_units,     area_units_);
    cout<<area_units<<endl;
    ...
    if (0 == ft) {
        ...
        cout<<"no database file called "<<database_file<<endl;
        return -1;
    ...
    for(i=0; i<n; i++) {
        ...
        if(flag==NULL) {
            cout<<"end of data after "<<i<<" entries"<<endl;
            break;
        }
        //cout<<entry[i]<<endl;
    }
    ...
    for(i=0; i<n; i++) {
        ...
        if(area_conversion_factor<0.0) {
            cout<<"GCR_data::read: error in converting units"<<endl;
            exit(0);
        }
        ...
        if(energy_conversion_factor<0.0) {
            cout<<"GCR_data::read: error in converting units"<<endl;
            exit(0);
        }
...
// so define the k.. values explicitly.
    ...
    for(i=0; i<n; i++) {
        ...
        if(strncmp(experiment[i],"AMS01"   ,5)==0) {
            color[i]=kRed ;
            style[i]=kOpenCircle;
        }
        if(strncmp(experiment[i],"JACEE"   ,5)==0) {
            color[i]=kRed ;
            style[i]=kOpenSquare;
        }
        if(strncmp(experiment[i],"BESS"    ,4)==0) {
            color[i]=kRed ;
            style[i]=kOpenTriangleUp  ;
        }
        if(strncmp(experiment[i],"IMP"     ,3)==0) {
            color[i]=kRed ;      //kOpenTriangleDown: produces dot
            style[i]=kFullDotLarge;
        }
        if(strncmp(experiment[i],"CAPRICE" ,7)==0) {
            color[i]=kRed  ;
            style[i]=kOpenStar  ;
        }
        if(strncmp(experiment[i],"RUNJOB"  ,6)==0) {
            color[i]=kRed ;
            style[i]=kOpenCross ;
        }

        if(strncmp(experiment[i],"HEAT"    ,4)==0) {
            color[i]=kBlue ;
            style[i]=kFullCircle;
        }
        if(strncmp(experiment[i],"HEAO3"   ,5)==0) {
            color[i]=kBlue;
            style[i]=kFullSquare;
        }
        if(strncmp(experiment[i],"SOKOL"   ,5)==0) {
            color[i]=kBlue;
            style[i]=kFullTriangleUp  ;
        }
        if(strncmp(experiment[i],"SANRIKU" ,7)==0) {
            color[i]=kBlue;
            style[i]=kFullTriangleDown;
        }
        if(strncmp(experiment[i],"IMAX"    ,4)==0) {
            color[i]=kBlue;
            style[i]=kFullStar ;
        }
        if(strncmp(experiment[i],"CRN"     ,3)==0) {
            color[i]=kBlue;      // kFullCross:no symbol plotted
            style[i]=kOpenDiamond;
        }
        if(strncmp(experiment[i],"ATIC-2"  ,6)==0) {
            color[i]=kCyan;
            style[i]=kOpenTriangleUp  ;
        }
        if(strncmp(experiment[i],"ATIC-1-2",8)==0) {
            color[i]=kCyan;      //AWS20081124
            style[i]=kOpenDiamond   ;
        }
        if(strncmp(experiment[i],"BETS"    ,4)==0) {
            color[i]=kRed ;      //AWS20081128
            style[i]=kMultiply      ;
        }
        if(strncmp(experiment[i],"PPB-BETS",7)==0) {
            color[i]=kRed ;      //AWS20081124
            style[i]=kMultiply      ;
        }
        if(strncmp(experiment[i],"HESS"    ,4)==0) {
            color[i]=kGreen;     //AWS20081126
            style[i]=kOpenSquare    ;
        }

        if(strncmp(experiment[i],"ACE"     ,3)==0) {
            color[i]=kCyan;
            style[i]=kPlus      ;
        }
        if(strncmp(experiment[i],"Voyager" ,7)==0) {
            color[i]=kMagenta;
            style[i]=kMultiply  ;
        }
        if(strncmp(experiment[i],"MUBEE"   ,5)==0) {
            color[i]=kGreen;
            style[i]=kOpenDiamond     ;
        }
...
int GCR_data::print()
{
    ...
    cout<<"selected original database entries  :";
    cout<<endl;
    for (i=0; i<n; i++)cout<<entry[i]<<endl;
    cout<<endl;
