...
int Galprop::gen_pi0_decay_emiss()     // generate pi0-decay emissivity
{
    ...
    if(iprotons==-1) {
        cout<<"  CR protons not found!"<<endl;
        ...
        for(int iEgamma=0; iEgamma<galaxy.n_E_gammagrid; iEgamma++) {
            ...
            for(int ip=0; ip<gcr[iprotons].n_pgrid; ip++) {
                ...
                if(galdef.integration_mode==1) { // ### old integration ###
                    if(gcr[iprotons].p[ip]<=Pth0) if(iHelium !=-1) {
                            if(gcr[iHelium].p[ip]/gcr[iHelium].A<=Pth0) continue;    //Gulli20070810
                    ...
                    if(gcr[iprotons].p[ip+1]<=Pth0) if(iHelium !=-1) {
                            if(gcr[iHelium].p[ip+1]/gcr[iHelium].A<=Pth0) continue;    //Gulli20070810
                        } else continue; //Gulli20070810
...
// Helium: derive y (= power-law index of the total expression)
    ...
    if(galdef.verbose==-702) { // selectable debug
        cout<<"   pi0-decay emissivity "<<endl;
        galaxy.pi0_decay_emiss.print();
    }
