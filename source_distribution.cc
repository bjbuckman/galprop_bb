...
//    zscale=0.200;  //nominal value AWS20131108 replaced by parameters[0]
    ...
    if (31 == srcModel) { //M82_b BJB20161215
        double zscale= parameters[0];
        double rscale= parameters[1];
        double cnorm = parameters[2];
        double z1scale = parameters[3];
        double r1scale = parameters[4];

        if (r <= rscale && fabs(z) <= zscale) {
            result = cnorm;
        } else if (r <= rscale && fabs(z) > zscale) {
            result = cnorm*exp(-(fabs(z)-zscale)/z1scale);
        } else if (r > rscale && fabs(z) <= zscale) {
            result = cnorm*exp(-(r-rscale)/r1scale);
        } else {
            result = cnorm*exp(-(r-rscale)/r1scale)*exp(-(fabs(z)-zscale)/z1scale);
        }
    }//M82_a
